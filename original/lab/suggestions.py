...
class Suggestion:
    ...
    target: str | None = None  # field being changed (e.g. "thresholds.no_action")
    current_value: float | None = None
    suggested_value: float | None = None
    expected_improvement: dict | None = None  # e.g. {"auc_delta": 0.03, "fpr_delta": -0.05}
...
def _threshold_at_max_fpr(sweep: dict, max_fpr: float) -> float | None:
    ...
    for t, fpr in zip(thresholds, fprs, strict=False):
        if fpr <= max_fpr:
            best = float(t)
    return best
...
def _threshold_for_constraints(
    ...
    if not sweep.get("thresholds"):
        return None
    best: float | None = None
    for t, tpr, fpr in zip(sweep["thresholds"], sweep["tpr"], sweep["fpr"], strict=False):
...
def _per_author_auc(individual_results: list[dict]) -> dict[str, float]:
    ...
    for author, rs in by_author.items():
        ...
        # Lower deviation = positive class (authentic), so AUC = P(pos < neg) --
        # computed directly from pairwise counts rather than a rank-sum U
        # statistic, which needs an extra sign-flip under this convention.
...
def generate_suggestions(
            ...
            if gap >= PER_AUTHOR_AUC_OUTLIER_GAP:
                suggestions.append(
                    Suggestion(
                        ...
                        rationale=(
                            "This author is harder to discriminate than the corpus "
                            "average. Consider: (1) more baseline samples from this "
                            "author, (2) a genre-specific anchor tier set, "
                            "(3) checking if their ghostwritten/authentic counts are balanced."
                        ),
    ...
    if corrections:
        ...
        if len(wrong) >= CORRECTIONS_DISAGREEMENT_MIN_N:
            ...
            divs = [
                c.get("original_divergence_score")
                for c in wrong
            ...
            if divs:
                ...
                suggestions.append(
                    Suggestion(
                        ...
                        title=(
                            f"{len(wrong)} corrections suggest the no_action "
