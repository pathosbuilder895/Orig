...
def run(
    *,
    ...
    print(f"[verify-null] eligible authors ({len(eligible)}): {eligible}", file=sys.stderr)
    ...
    print(f"[verify-null] {len(baseline_pairs)} pairs in {elapsed_s:.1f}s", file=sys.stderr)
    ...
    write_report(
        paths,
        label=f"{label}_nullmodel_N{baselines}",
        report=llr_report,
        env_lock=ENV_LOCK,
    ...
    print(
        f"│  baseline  median per-author AUC: {baseline_report.median_per_author_auc:.4f}"
        f"  IQR [{baseline_report.iqr_per_author_auc[0]:.4f}, {baseline_report.iqr_per_author_auc[1]:.4f}] │"
    )
    print(
        f"│  impostor  median per-author AUC: {llr_report.median_per_author_auc:.4f}"
        f"  IQR [{llr_report.iqr_per_author_auc[0]:.4f}, {llr_report.iqr_per_author_auc[1]:.4f}] │"
    )
    print(
        f"│  Δ median AUC: {llr_report.median_per_author_auc - baseline_report.median_per_author_auc:+.4f}"
        f"                                            │"
    )
    print(
        f"│  baseline  pooled-uncalibrated AUC: {baseline_report.pooled_uncalibrated_auc:.4f}    │"
    )
