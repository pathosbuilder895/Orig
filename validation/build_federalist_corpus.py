...
def save_corpus(papers: Dict[int, str], corpus_dir: Path) -> Dict[int, str]:
    ...
    for num, text in papers.items():
        ...
        tag = {"H": "hamilton", "M": "madison", "J": "jay", "HM": "joint", "D": "disputed"}.get(
            auth_code, "unknown"
        )
...
def build_manifest(
    ...
    madison_scoring = [
        n for n in madison_undisputed if n not in BASELINE_PAPERS["madison"] and n in filenames
    ][:6]
    ...
    hamilton_scoring_cross = [
        n for n in hamilton_undisputed if n not in hamilton_baseline and n in filenames
    ][2:8]
...
def main():
    ...
    for label, stats in sorted(report.per_label_stats.items()):
        print(
            f"  {label:14s}  mean={stats['mean_deviation']:.3f}  "
    ...
    if disputed_results:
        ...
        for r in sorted(disputed_results, key=lambda x: x.filename):
            ...
            print(
                f"  No. {num:>2s}  prob={r.authorship_probability:.3f}  "
