...
def main() -> int:
    ...
    for aid in eligible:
        ...
        for entry in by_author[aid]["baseline"]:
            ...
            if r.status_code != 200:
                print(f"  ⚠ baseline {aid} {entry['filename']}: {r.status_code}", file=sys.stderr)
        print(f"  baseline {aid}: {len(by_author[aid]['baseline'])} samples", file=sys.stderr)
        ...
        for entry in by_author[true_author]["scored"]:
            ...
            for mode in ("off", "on"):
                ...
                for candidate in eligible:
                    rr = client.post(
                        f"/students/demo:pa_{candidate}/score",
                        json={
                            "text": text,
                            "assignment": entry["prompt"],
    ...
    summary["delta_mean_rank"] = round(summary["on"]["mean_rank"] - summary["off"]["mean_rank"], 3)

    payload = {"summary": summary, "rows": rows, "eligible": eligible, "env": ENV_LOCK.__dict__}
    ...
    print(
        f"│  flag OFF →  top1={summary['off']['top1']:.2%}   mean_rank={summary['off']['mean_rank']:.2f}     │"
    )
    print(
        f"│  flag ON  →  top1={summary['on']['top1']:.2%}   mean_rank={summary['on']['mean_rank']:.2f}     │"
    )
