...
// Tier 18 (uniformity) ships disabled by default: it is built and tested, but
// its fairness and paraphrase-resistance gates have not been cleared, so the
// prototype must not present it as a signal a professor can act on today.
const PENDING_VALIDATION_TIER = 18;
const COMPARISON_TIER = 0;
const liveCaptureCodes = CODES.filter((code) => FEATURES[code].tier === LIVE_CAPTURE_TIER);
const pendingValidationCodes = CODES.filter((code) => FEATURES[code].tier === PENDING_VALIDATION_TIER);
 ...
 * signals, which require live keystroke capture, and Tier 18's six uniformity
 * signals, which are disabled pending their calibration gates. Values
 * displayed by the prototype are intentionally synthetic examples, never
 * measured student data.
 */
export const FEATURE_UNIVERSE = Object.freeze({
  totalCount: CODES.length,
  defaultActiveCount: CODES.length - liveCaptureCodes.length - pendingValidationCodes.length,
  liveCaptureOnlyCount: liveCaptureCodes.length,
  pendingValidationCount: pendingValidationCodes.length,
  pendingValidationCodes: Object.freeze([...pendingValidationCodes]),
...
export const SYSTEMS = Object.freeze([
  {
    ...
    tiers: Object.freeze([1, 4, 5, 6, 7, 8, 11, 13, 14, 15, 17, 18, 0]),
...
const TIER_DESCRIPTIONS = {
  ...
  18: 'How much the paper varies from itself — the spread of sentence length, punctuation, vocabulary, and clause depth. Held back pending its fairness and paraphrase-resistance gates.'
};
...
export const TIERS = Object.entries(TIER_META)
  .map(([tier, name]) => {
    ...
    const pendingValidation = number === PENDING_VALIDATION_TIER;
    ...
    return {
      ...
      status: requiresLiveCapture ? 'live capture required' : pendingValidation ? 'pending validation' : requiresBaseline ? 'baseline comparison' : 'available',
      availability: requiresLiveCapture ? 'live-capture-only' : pendingValidation ? 'pending-validation' : requiresBaseline ? 'baseline-comparison' : 'imported-document',
      availableInDefaultPilot: !requiresLiveCapture && !pendingValidation,
      requiresLiveCapture,
      pendingValidation,
      requiresBaseline,
...
const SHORT_LABELS = [
  ...
  'Typing smoothness','Burst typing share','Backspace use','Long pauses count','Paste frequency','Edit size',
  'Sentence length sameness','Section‑to‑section sameness','Glue‑word clumping','Punctuation sameness','New word pacing evenness','Clause depth sameness',
  'Letter habit drift','Glue‑word drift'
];
...
export const FEATURE_PLANETS = CODES.map((code, index) => {
  ...
  const pendingValidation = meta.tier === PENDING_VALIDATION_TIER;
  ...
  const evidence = requiresLiveCapture
    ...
    : pendingValidation
      ? {
          current: 'Not enabled · calibration gates outstanding',
          baseline: 'No validated baseline band yet',
          reading: `${meta.name} tracks ${meta.plain}. It is measured but withheld until its fairness and paraphrase-resistance gates clear, so no value is shown.`
        }
  ...
  return {
    ...
    sentiment,
    signalStatus,
    status: requiresLiveCapture ? 'live capture required' : pendingValidation ? 'pending validation' : requiresBaseline ? 'baseline comparison' : signalStatus,
    availability: requiresLiveCapture ? 'live-capture-only' : pendingValidation ? 'pending-validation' : requiresBaseline ? 'baseline-comparison' : 'imported-document',
    availableInDefaultPilot: !requiresLiveCapture && !pendingValidation,
    availableInImportedDocument: !requiresLiveCapture && !pendingValidation,
    requiresLiveCapture,
    pendingValidation,
    requiresBaseline,
    evidenceProvenance: 'synthetic-demonstration',
    measurementStatus: requiresLiveCapture ? 'not-captured' : pendingValidation ? 'not-enabled' : 'illustrative',
    isMeasuredStudentData: false,
