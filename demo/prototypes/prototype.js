...
function pulse() {
  return shell(`${header('Future concept · Relationship explorer','Writing-pattern galaxy','Illustrative map · 109 traits')}
    <section class="pulse-layout">
      <div class="pulse-stage panel">
        <div class="pulse-topbar">
          <div><span class="pulse-live"><i></i>ILLUSTRATIVE MODEL</span><b>Three writing-division solar systems · 109 feature planets</b></div>
      ...
      <aside class="pulse-controls">
        <div class="panel interest-panel"><div class="panel-label"><span data-index-title>THREE WRITING DIVISIONS</span><b data-index-count>109 FEATURE PLANETS</b></div><p data-index-copy>Choose the Surface, Discourse, or Rhetoric sun. Its tier planets appear next, followed by concrete current and baseline examples.</p>
...
function featureCatalog() {
  ...
  const availability = feature => feature.tier === 17 ? 'Needs drafting data' : feature.tier === 18 ? 'Pending validation' : feature.tier === 0 ? 'Baseline only' : 'Available';
  ...
  return shell(`${header('Professor demonstration · Passage review','Living document intelligence','Morgan Lee · Fictional Essay 4 · Draft 04')}
    <section class="editor-shell${reduceEditorBackground?' background-reduced':''}" data-editor-theme="navy">
      ...
      <aside class="feature-library glass-panel">
        <div class="editor-panel-head"><div><span>AUTHORSHIP FEATURES</span><b>109 total · 97 available · 6 need live drafting · 6 pending validation</b></div><button data-feature-all aria-label="Show all feature overlays">∞</button></div>
        <div class="catalog-mode" role="group" aria-label="Feature catalog depth"><button type="button" data-catalog-mode="essentials" aria-pressed="${professorCatalogMode==='essentials'}">Professor essentials</button><button type="button" data-catalog-mode="all" aria-pressed="${professorCatalogMode==='all'}">All 109 traits</button></div>
