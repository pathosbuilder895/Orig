...
from tests.certification.conftest import MIN_AUTHORS, provision_and_score, record_verdict
...
def test_same_author_action_at_pilot_baseline_counts(
    ...
    Each author's baselines come from one work and the scored holdout from a
    different work by the same author, so every flag here is a false positive
    on genuine authorship. Through the API (baselines and score both), default
    flags, three-valued: below MIN_AUTHORS usable authors the verdict is
    `uninformative` and the test skips rather than passing on noise.
...
def test_harness_flags_a_different_author(live_client, store_reset, corpus_authors):
    """Witness for T-01: the same harness flags impostor holdouts.
    ...
    *next* author's holdout. If this were also unflagged the certification
    ...
    Green on this branch; not a blocker.
    """
    n_baselines = 5
    eligible = [a for a in corpus_authors if len(a.baseline_chunks) >= n_baselines]
    if len(eligible) < MIN_AUTHORS:
    ...
    record_verdict(
        "T-01 witness: impostor holdout",
        "pass" if rate >= MIN_WITNESS_FLAG_RATE else "fail",
        value=rate,
        n=len(usable),
        n_baselines=n_baselines,
