...
class TestSuggestionEngine:
    ...
    def test_overlapping_data_yields_lower_auc(self):
        ...
        report = _synthetic_report(n_pos=20, n_neg=10, pos_mean=0.50, neg_mean=0.55)
    ...
    def test_per_author_outlier_detection(self):
        ...
        for i in range(5):
            results.append(
                {
                    "filename": f"a0_auth_{i}.txt",
                    "author_id": "author_0",
                    "label": "authentic",
                    "deviation_score": 0.10 + 0.01 * i,
                    "authorship_probability": 0.90,
                    "recommended_action": "no_action",
                    "is_same_author": True,
                    "word_count": 1000,
                    "scoring_time_ms": 1.0,
        ...
        for i in range(3):
            results.append(
                {
                    "filename": f"a0_ghost_{i}.txt",
                    "author_id": "author_0",
                    "label": "ghostwritten",
                    "deviation_score": 0.90 - 0.01 * i,
                    "authorship_probability": 0.10,
                    "recommended_action": "escalate",
                    "is_same_author": False,
                    "word_count": 1000,
                    "scoring_time_ms": 1.0,
        ...
        for i, s in enumerate(a1_pos):
            results.append(
                {
                    "filename": f"a1_auth_{i}.txt",
                    "author_id": "author_1",
                    "label": "authentic",
                    "deviation_score": s,
                    "authorship_probability": 1 - s,
        ...
        out = generate_suggestions(report)
        outliers = [s for s in out["suggestions"] if s["type"] == "per_author_outlier"]
    ...
    def test_corrections_disagreement_signal(self):
        ...
        corrections = [{"is_correct": False, "original_divergence_score": 0.80} for _ in range(5)]
...
class TestCalibrationRunStore:
    ...
    def test_list_filters(self, client_module_db):
        ...
        module.store.complete_calibration_run(
            r1, auc=0.9, n_essays_scored=10, n_authors=2, report={}
        )
        # r2 stays "running"
