...
def _resolver_outputs(
    *,
    ...
    return {
        "language": {"primary": "en", "segments": {"en": 1.0}, "code_switched": code_switched},
        "genre": {"primary": genre, "confidence": 0.5, "secondary": None},
        "topic": {"domain": "unknown", "baseline_distance": 0.1, "novelty": novelty},
        "length": {
            "tokens": 1000,
            "regime": length_regime,
            "reliable_tiers": [],
            "suppress_tiers": [],
        },
        "citations": {
...
class TestGenre:
    def test_creative_fiction_mutes_t16(self):
        ...
        # _derive_directives.
        d = _derive_directives(_resolver_outputs(genre="creative_fiction", citations_present=True))
        muted = set(d["weight_modifications"]["mute_codes"])
        assert set(TIER16_CODES).issubset(muted)
...
class TestDefaults:
    ...
    def test_mute_overrides_attenuate(self):
        ...
        d = _derive_directives(
            _resolver_outputs(length_regime="micro", composition_mode="tool_cleaned")
        )
        muted = set(d["weight_modifications"]["mute_codes"])
        attenuated = set(d["weight_modifications"]["attenuate_codes"])
