...
|---|---|---|---|---|---|---|
| 1 | `2026-09-07-mvp-launch-part1-coverage-landing.md` | Land the completed branch-coverage effort; CI → `--cov-branch --cov-fail-under=98` | user's Mac | — | post-merge CI green on origin/main under the new gate | **done @ CI gate `--cov-branch --cov-fail-under=98`, coverage 99.56%** — coverage effort + gate merged (#194/#195/#196, 2026-09-07); one straggler: main CI went red on 3 soak-reader tests whose fixtures were left untracked, fixed here |
...
| 3 | `2026-09-07-mvp-launch-part3-soak-readers.md` | Build `scripts/fused_shadow_report.py` + `scripts/characteristic_soak_report.py` with tests | cloud session | — | both readers green on fixtures; suite 0 failed; PILOT_RUNBOOK cites them | **done @ unified `scripts/shadow_soak_report.py`** — both named channels (fused C1 confound, characteristic dispersion) landed as one reader via #195 (Task 0 collapse); committed its untracked fixtures, added a read-only-connection guard + test, wired PILOT_RUNBOOK §3d |
