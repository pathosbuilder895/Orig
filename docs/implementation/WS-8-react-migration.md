...
## Acceptance criteria
> Verified against the working tree 2026-07-09. R0 scaffold + R1 (Bluebook ESM fold-in) landed (`9e624d5`); R2/R3/R4 not started.
- [ ] **Every R2 primitive and every R3 page PR passes axe with zero serious/critical violations** — NOT DONE. No R3 page cutover exists yet (`run.py` still mounts `demo/` directly; no page consumes `app/dist`).
- [ ] All durable a11y findings closed at the React root, mapped to their fix: W5→`LabeledInput`; W6/W14→`tokens.css`; W7→heading primitives + `DataTable`; W8→`Modal` + route focus/title mgmt; W11→reduced-motion tokens; W12→`role=switch`/tab roles; W13→`FileDrop`; W15→`SkipLink`. (Durable slices of W1→`NavList`, W4→`LiveRegion`, W9→`Chart`, W10→`Timer` also closed.) — NOT DONE. `app/src/components/` currently has only `SkipLink.tsx` (W15); `LabeledInput`/`Modal`/`LiveRegion`/`DataTable`/`NavList`/`FileDrop`/`tokens.css`/reduced-motion are not yet built.
- [x] Bluebook's no-CDN-at-exam-time invariant holds **by construction** (React bundled as an ESM dep, not vendored-global) — R1. (Today it already holds in *prod* via committed `vendor/` + a Playwright assertion; bundling makes it structural and brings *dev* — currently on the unpkg CDN — onto the same React.) — DONE: `demo/bluebook/vendor/` and `index.prod.html` are gone; `build.mjs` bundles `react`/`react-dom` as real dependencies; all Bluebook `.jsx` files use `import`/`export`, no CDN in dev or prod.
- [ ] The committed-`dist/` deploy pattern is generalized from Bluebook and protected by the B2 freshness gate; CI builds, artifact ships committed (Render has no Node). — PARTIAL: the B2 gate covers `bluebook.bundle.js` today; a separate `app` CI job lints/typechecks/builds, but `app/dist` is gitignored and nothing consumes or freshness-checks it yet (expected — no R3 page exists to serve it).
- [ ] Every live page served from React at its original path; `student-coach.html` and `landing.html` retired (F5); `?legacy=1` fallbacks removed after their release window. — NOT DONE, correctly so per this doc's own sequencing (R4 retirement is gated on R3, which hasn't started); both files still exist and `student.html` still calls `openCoach()`.
- [ ] WS-9 Playwright suite green on the React stack, incl. new professor-side coverage (T7). — PARTIAL: `professor-journey.spec.mjs` exists and is substantive, but it exercises the legacy (pre-React-cutover) Bluebook bundle, not "the React stack" — there is no React stack serving a page yet.
