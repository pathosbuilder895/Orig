...
- [x] Total collected E2E tests ≈ **55–70** (`npx playwright test --list`). — MET as of 2026-08-13: **67** explicit `test()` calls across **14** spec files (`grep -c "test(" demo/bluebook/e2e/*.spec.mjs`), plus `a11y.spec.mjs`'s loop-generated screen scans on top. (The 2026-07-09 snapshot of 38/9 predates the exam-robustness, corrections-UI, proctor phone-park, and dev-tools-gate suites.)
