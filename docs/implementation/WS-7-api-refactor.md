...
## Acceptance criteria
- [ ] `grep -n "os.environ" original/quantum/scoring.py` → **0**; `score()` takes `config`, `genre_stats`, `authentic_fidelities`; no call-time `from ..store` import remains (A5 edge deleted).
- [ ] WS-5 flags-OFF byte-identical `score()` assertion passes at every WS-7 commit (Phase-1 scoring math unchanged).
- [ ] `grep -n "body: dict" original/api.py` → **0**; all 8 endpoints have request models in `schemas.py`; malformed body → 422; `_to_response` typed + built via `model_validate(asdict(...))` with a passing round-trip test.
- [ ] `api.py` split into `routers/{auth,lti,bluebook,students,scoring,tenants,admin,lab}.py` + `services/scoring_service.py`; route inventory (method, path, response body) **identical** before/after; `python run.py --demo` boots.
- [ ] The 4 file/CSV upload handlers no longer block the loop (`/health` responsive under a multi-PDF batch).
- [ ] `CONTEXT_MANIFEST_ENABLED`/`ADAPTIVE_WEIGHTS_ENABLED`/`NULL_MODEL` declared GA; off-branches removed; redundant `run.py` setdefaults gone; `AMPLITUDE`/`BAYESIAN`/`AI_LIKELIHOOD` remain flags.
- [ ] One env var governs deploy tier (ORIGINAL_ENV/ENVIRONMENT merged via promoted `Settings`); CLAUDE.md flag table lists every behavior-affecting flag.
- [ ] `operator.html`/`admin-context.html` gated on real deploys; `landing.html` deleted; Canvas stubs → 501; `run.py` banner fixed.
- [ ] Tier registry: one `TIERS` tuple drives `constants.py` + `pipeline.py`; `ALL_FEATURE_CODES` byte-identical to pre-refactor (len 103).
- [ ] Full suite `.venv/bin/python -m pytest tests/ -q` → **0 failed** after every step.
