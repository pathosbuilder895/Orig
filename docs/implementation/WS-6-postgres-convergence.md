...
## Acceptance criteria
> Verified against the working tree 2026-07-09. P1 (seam-widening) has landed; P0 and P2–P6 have not started.
...
- [ ] **v1 dissolved** — `original/api/`, `canvas/`, `main.py`, `middleware/`, `auth/`, `schemas_v1/`, `tasks/` + 62 v1 tests deleted; `run.py` importlib hack gone; `frontend/`/`web/`/`legacy_mvp/` removed; Docker/fly targets retargeted (F1, F4, F6, B15, A9, T4, T6). — NOT DONE (P6, not started): `original/api/`, `original/main.py`, `original/canvas/`, `original/middleware/`, `original/db/`, `alembic/`, `frontend/`, `web/` all still present; WS-5 only did the *interim* `test_api.py`→`test_v1_api_dormant.py` rename, not the final P6 deletion.
