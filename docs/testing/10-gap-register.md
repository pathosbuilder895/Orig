...
| T-09 | Bulk upload blocks the event loop; live exam heartbeats stall | B4 | 07 §2 | S test / S fix | heartbeat < 250 ms during upload — closed on main (post-#203 rebase converted `upload_file`, `upload_baseline_batch`, `import_turnitin_csv` from `async def` to threadpooled `def`); `tests/perf/test_event_loop_not_blocked.py` now passes unmarked as a regression guard | closed on main |
