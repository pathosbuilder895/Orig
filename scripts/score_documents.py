...
def score_one(client, student_id: str, path: Path) -> DocScore:
    ...
    try:
        ...
        return DocScore(
            ...
            authorship_probability=float("nan"),
            recommended_action="error",
            catastrophic_drift=False,
            catastrophic_drift_rms_z=0.0,
            scoring_time_ms=0.0,
            error=str(e),
        )
    if not text.strip():
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    g.add_argument(
        "--baseline-dir", type=Path, help="Directory of baseline files (PDF/DOCX/TXT/MD)."
    )
    g.add_argument("--baseline-file", type=Path, help="Single baseline file.")
    g.add_argument(
    ...
    s.add_argument("--score-dir", type=Path, help="Directory of submission files to score.")
    s.add_argument("--score-file", type=Path, help="Single submission file to score.")

    ap.add_argument(
        "--student-id",
        default="cli_test_student",
    ...
    ap.add_argument("--csv", type=Path, help="Optionally also write a flat CSV report.")
    ...
    if args.use_existing_student:
        ...
        if args.baseline_dir:
            ...
            print(
                "ERROR: either --baseline-dir/--baseline-file or --use-existing-student is required",
                file=sys.stderr,
            )
            return 2
        if not baselines:
        ...
        if not baseline_summary["added"]:
            ...
            return 1
        print(
            f"  {len(baseline_summary['added'])} baseline samples added, "
            f"{len(baseline_summary['failed'])} failed",
            file=sys.stderr,
        )
        ...
        json.dumps(
            {
                ...
                "summary": {
                    ...
                    "n_flagged": sum(
                        1 for s in scores if s.recommended_action not in ("no_action", "monitor")
                    ),
                    "n_catastrophic_drift": sum(1 for s in scores if s.catastrophic_drift),
                    "mean_deviation": (
                        round(
                            sum(s.deviation_score for s in scores if s.error is None)
                            / max(1, sum(1 for s in scores if s.error is None)),
                            4,
                        )
                    ),
                },
