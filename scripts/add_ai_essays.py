...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "essay_dir", type=Path, help="Directory of .txt essays to ingest (sorted by name)."
    )
    ap.add_argument(
        "--provider",
        required=True,
    ...
    ap.add_argument(
        "--author-id",
        default="ai_author",
        help="Manifest author_id (default: ai_author, pooled with " "the existing AI essays).",
    )
    ap.add_argument(
        "--prompt-map",
        type=Path,
    ...
    ap.add_argument(
        "--default-prompt",
        default="AI-generated theology essay",
        help="Prompt used for files not in --prompt-map.",
    )
    ap.add_argument(
        "--notes", default=None, help="Notes string (default records provider + ingest date)."
    )
    ...
    for src in sources:
        ...
        if word_count < MIN_WORDS_WARN:
            print(
                f"[add-ai-essays] WARN: {src.name} is only {word_count} words "
                f"(< {MIN_WORDS_WARN}) — short texts weaken the eval",
                file=sys.stderr,
            )
        ...
        if filename in existing_names or (args.corpus_dir / filename).exists():
            print(
                f"[add-ai-essays] FAIL: {filename} already exists — refusing " f"to overwrite",
                file=sys.stderr,
            )
            return 1
        ...
        print(
            f"[add-ai-essays] {src.name} → {filename} "
            f"({word_count} words, provider={args.provider})"
        )

    if args.dry_run:
        print(
            f"[add-ai-essays] dry-run: {len(new_entries)} essay(s) validated, " f"nothing written."
        )
        return 0
    ...
    print(
        f"[add-ai-essays] added {len(new_entries)} essay(s) to "
